...
def build(out):
    ...
    entry = [
        ["Шаг", "Действие", "Детали"],
        ["1", "Открыть счёт у брокера", "Interactive Brokers (рекомендация): все тикеры есть, "
         "комиссия €1.25/сделка EU, Modelo 720 отчёт скачивается. "
         "Альтернатива: DEGIRO (дешевле, но нет PHPD/PHPT)."],
        ["2", "Перевести €300 000", "SEPA-перевод с банковского счёта → IB. Обычно T+1."],
        ...
        ["4", "Проверить Modelo 720", "Порог €50k пробит сразу. Декларация до 31 марта "
         "следующего года. BTCE идёт в 720 (не 721)."],
    ...
    costs = [
        ...
        ["Брокерские комиссии*", "€50", "€50", "€50", "≈ €500"],
        ["Инфляционные потери банк €800k**", "€20 000", "€20 000", "€20 000", "€200 000"],
        ["ИТОГО drag", "€35 090", "€44 530", "€58 900", "≈ €476 500"],
    ]
    s.append(tbl(costs, [52*mm, 22*mm, 22*mm, 22*mm, 25*mm]))
    p("<i>* IB: ~€1.25/сделка × 14 позиций × ~3 покупки/год ≈ €50. "
    ...
    p("Действие 5: Выбор брокера", H3)
    broker = [
        ["Брокер", "Плюсы", "Минусы", "Все 14 тикеров?"],
        ["Interactive Brokers", "Все тикеры, €1.25/сделка, "
         "выгрузка для 720", "Сложный интерфейс", "Да"],
        ["DEGIRO", "Бесплатные ETF (core selection), "
         "простой UI", "Нет PHPD/PHPT, нет BTCE", "Нет (11/14)"],
        ["Scalable Capital", "€0.99/сделка, XETRA доступ", "Нет ETC драгметаллов", "Нет (10/14)"],
    ]
    s.append(tbl(broker, [30*mm, 52*mm, 40*mm, 30*mm]))
    p("<b>Рекомендация: Interactive Brokers.</b> Единственный, где "
      "доступны все 14 позиций + BTCE на XETRA + Modelo 720 export.", B)
    sp()
    ...
    summary = [
        ...
        ["Брокер IB (vs дорогой банк)", "1 день на миграцию", "+€5 000–10 000 (vs банковский 0.5–1% сделка)"],
    ]
